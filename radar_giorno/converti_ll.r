...
lev<-read.table("/home/meteo/radar_giorno/level_swiss.txt",na.strings="NA")
lev<-lev$V2
...
clutter<-raster("/home/meteo/radar_giorno/clutter_all.gri")
...
#	llr<-raster(nrows=rows,ncols=cols,xmn=8,xmx=(8+cols*0.01),ymn=44.0,ymx=(44.0+rows*0.01),crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0"))
...
#	otfile=paste(otdir,"/",substr(infile[i],31,(nchar(infile[i])-5)),sep="")
